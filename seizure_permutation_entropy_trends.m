function [Params]=seizure_permutation_entropy_trends(dir_in,T,Tw,To,m);

% Returns trends of RR-sequence (2 Hz interpolated) Permutation Entropy of all seizures in files in folder DIR_IN, 
% starting from T sec. before seizure using the time window Tw sec. 
% moved with overlapping To sec. M is the PermEn order.
...
%         dummy2=zeros(1,W);% array for trend2
        for w=1:W
            dummy1(w)=perm_entropy_value(q(:,w),m);
%             dummy2(w)=apen(q(:,w),M,.01*std(q(:,w)));
        end
        hrv_data_before{kb,1}=dummy1;
%         hrv_data_before{kb,2}=dummy2;
...
%         params2(s,w)=hrv_data_before{s,2}(w);
    end
end
...
%     nzi=find(params2(:,w));%indicies of nonzero elements for each window
%     Param2{w}=params2(nzi,w);
end
