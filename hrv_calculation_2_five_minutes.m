...
% before seizure
T=.1*60;% seconds before 
m=3;% Permutation Entropy order
...
for i=1:length(fileStruct)% all mat files
    ...
    for s=1:S; % for every seizure in the signal
        ...
        %%%%%%%%%
        %%% before seizure
        if d.seizure(s).time_relBefore(1)>=T;
            disp('Enough time BEFORE!')
            st_ind=find(d.seizure(s).time_relBefore<=T);
            st_ind=st_ind(1);% start of T-sec. window
%             [hrv_data_before(kb,1), hrv_data_before(kb,2),...
%                 hrv_data_before(kb,3), hrv_data_before(kb,4),...
%                 hrv_data_before(kb,5)]=hrv_for_interpolated_signal(d.seizure(s).RRBefore(st_ind:end),...
%                 d.Fs);
            
%             subplot(2,1,1);
%             plot(d.seizure(s).RRBefore(st_ind:end));
%             title(['before'])
            hrv_data_before(kb,6)=perm_entropy_value(d.seizure(s).RRBefore(st_ind:end),m);
            hrv_data_before(kb,7)=apen(d.seizure(s).RRBefore(st_ind:end),...
                M, .01*std(d.seizure(s).RRBefore(st_ind:end)));
            
            
            kb=kb+1;% seizure counter
        end
        %%% After  seizure
        if d.seizure(s).time_relAfter(end)>=T;
            disp('Enough time AFTER!')
            st_ind=find(d.seizure(s).time_relAfter>=T);
            st_ind=st_ind(1);% end of T-sec. window
%             [hrv_data_after(ka,1), hrv_data_after(ka,2),...
%                 hrv_data_after(ka,3), hrv_data_after(ka,4),...
%                 hrv_data_after(ka,5)]=hrv_for_interpolated_signal(d.seizure(s).RRAfter(1:st_ind),...
%                 d.Fs);
%             subplot(2,1,2);
%             plot(d.seizure(s).RRAfter(1:st_ind));
%             title(['after'])
            hrv_data_after(ka,6)=perm_entropy_value(d.seizure(s).RRAfter(1:st_ind),m);
            hrv_data_after(ka,7)=apen(d.seizure(s).RRAfter(1:st_ind),...
                M,.01*std(d.seizure(s).RRAfter(1:st_ind)));
