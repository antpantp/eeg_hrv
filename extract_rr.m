...
record = 'ECG_only_gudz-5_1.mat'; % data file
% record = 'D:\Google_Drive\hrvepi\ECG\single_seizure_ECG_2_seizures_boyko-18.mat'; % data file
...
maf_dth = 0.5; % s, deviation threshold from moving average 
...
for i = 1:frame_len:length(d.data) 
  i_end = min(i+frame_len,length(d.data));
  fprintf(1,'Processing frame (%d:%d).... \n', i, i_end);  
  ecg = d.data(i:i_end)*1e-3; % mV
  
  % Find QRS complexes...
  i_qrs_frame = findqrs(ecg,d.Fs,1,'ref_lead',1);   
  drawnow();
